...
> 本文件是交付给实施 Agent 的**完整任务书**。所有「仓库事实」均已逐条核对源码，标注了 `文件:行号`，**可直接采信，不需要重新验证**。
> 与本文件冲突的任何先验知识（含你对 LangChain4j / Vue / Spring Boot 的记忆）以本文件为准。
...
| 事实 | 值 | 证据 |
|------|-----|------|
| 模块 | `sky-common` / `sky-pojo` / `sky-server`，父 `com.sky:sky-take-out:1.0-SNAPSHOT` | `sky-take-out/pom.xml:11-18` |
| Spring Boot | **2.7.3** | `sky-take-out/pom.xml:6-9` |
| **Java 编译级别** | **1.8**（全仓库未声明 `java.version`，继承 `spring-boot-starter-parent:2.7.3` 默认 `1.8`） | 无覆盖声明 |
| 统一响应 | `com.sky.result.Result<T>`：`code=1` 成功，`code=0` 失败；字段 `code/msg/data` | `sky-common/.../result/Result.java:16-43` |
| 管理端前缀 | 全部 `/admin/xxx`（category/common/dish/employee/order/report/setmeal/shop/workspace） | 9 个 Controller 的 `@RequestMapping` |
| `server.port` | **8080**，**无 `context-path`** | `application.yml:1-2` |
| JWT 头名 | `sky.jwt.admin-token-name: token` → 请求头就叫 **`token`** | `application.yml:39-45` |
| 拦截器 | `JwtTokenAdminInterceptor` 解析 JWT 取 claim `empId`，`BaseContext.setCurrentId(empId)`；失败 `response.setStatus(401); return false`（**无响应体**） | `interceptor/JwtTokenAdminInterceptor.java:44-64` |
| 拦截范围 | `addPathPatterns("/admin/**").excludePathPatterns("/admin/employee/login")` | `config/WebMvcConfiguration.java:53-57` |
| `BaseContext` API | `setCurrentId(Long)` / `getCurrentId()` / `removeCurrentId()` | `sky-common/.../context/BaseContext.java` |
| 全局异常 | `GlobalExceptionHandler` 只处理 `BaseException` 和 `SQLIntegrityConstraintViolationException`，**没有兜底 `Exception` handler** | `handler/GlobalExceptionHandler.java:33-36` |
| 异常体系 | `com.sky.exception.BaseException extends RuntimeException`；子类含 `OrderBusinessException`、`DeletionNotAllowedException` 等 | `sky-common/.../exception/` |
| 订单状态常量 | `Orders.PENDING_PAYMENT=1, TO_BE_CONFIRMED=2, CONFIRMED=3, DELIVERY_IN_PROGRESS=4, COMPLETED=5, CANCELLED=6`；支付状态 `UN_PAID=0, PAID=1, REFUND=2` | `sky-pojo/.../entity/Orders.java:21-36` |
| 已有依赖 | web / mybatis-spring-boot-starter 2.2.0 / druid 1.2.1 / pagehelper 1.3.0 / data-redis / cache / websocket / fastjson 1.2.76 / lombok / springdoc-openapi-ui 1.8.0 / poi 3.16 | `sky-server/pom.xml` |
| 已有流式技术 | **只有 WebSocket**（`websocket/WebSocketServer.java`，端点 `/ws/{sid}`），**没有任何 SSE / AI / LangChain 代码** | 全仓库检索 |
...
void                confirm(OrdersCancelDTO dto);                    // ⚠ 入参是 OrdersCancelDTO
...
| 事实 | 值 | 证据 |
|------|-----|------|
| 技术栈 | Vue **2.6.10**、vue-router 3.1.3、Vuex 3.1.2、`vuex-module-decorators`、Element UI **2.13.0**、`vue-property-decorator` 8、axios 0.19.0、TypeScript **3.6.2**（精确锁定） | `package.json` / `package-lock.json` |
| dev 端口 | **8081** | `vue.config.js:21-25` |
| 代理 | `/api` → `process.env.VUE_APP_URL`，`pathRewrite: {'^/api': ''}` | `vue.config.js:31-41` |
| `VUE_APP_URL`(dev) | `http://localhost:8080/admin` | `.env.development:15` |
| `VUE_APP_BASE_API` | `/api` | `.env.development:2` |
| ⇒ 结论 | 前端 `POST /api/agent/stream` = 后端 `POST http://localhost:8080/admin/agent/stream` ✅ | — |
| 路由 | 单文件 `src/router.ts`，侧栏 = `path:"/"`(Layout) 的 `children` | `src/router.ts:40-149` |
| 侧栏渲染 | `SidebarItem.vue` 渲染 `<i class="iconfont" :class="meta.icon" />` | `layout/components/Sidebar/SidebarItem.vue:22-26` |
| **图标真相** | `meta.icon` 是 **iconfont CSS 类名**，不是 svg 名。可用类：`icon-category / icon-employee / icon-user / icon-order / icon-combo / icon-lock / icon-dish / icon-inform / icon-statistics / icon-clear / icon-download / icon-open / icon-stop / icon-waiting / icon-staySway / icon-all / icon-cancel / icon-complete` | `src/styles/icon/iconfont.css` |
| 权限 | `permission.ts` 只校验 cookie `token` 是否存在，**无角色过滤、无 addRoutes** ⇒ 加一条 child route 即可出现在侧栏 | `src/permission.ts:11-21` |
| token 存储 | `js-cookie`，key = `'token'`，`getToken()` 在 `src/utils/cookies.ts:15-18` | — |
| axios 去重 | `requestOptimize.ts` 用 `md5(url+method+body)` 做 in-flight 去重，**会取消重复 POST** | `utils/requestOptimize.ts:3-23` |
| tsconfig | `strict: false`，`target: esnext`，**`exclude: ["src/**/*.vue"]`** | `tsconfig.json` |
| Element UI | `main.ts` 已 `Vue.use(ElementUI)`，`this.$message` / `$confirm` / `$notify` 全局可用 | `src/main.ts:24` |
| 关键 lint 规则 | `vue/match-component-file-name: 'error'` ⇒ **组件 `name` 必须与文件名一致**；`eqeqeq`/`semi`/`indent` 已关闭；无 `max-len` | `.eslintrc.js` |
| 现有 fetch/SSE | **无**。全部走 axios；仅 Navbar 用了 WebSocket | 全仓库检索 |

---

## 3. ⚠ 原始需求中的 6 个错误（已核实，**按更正后的执行**）

实施时若你「记得」下面左列的说法，请一律以右列为准：

| # | 原说法（错误） | 核实后的事实 | 影响 |
|---|---------------|-------------|------|
| E1 | 存在 `com.sky.service.ShopService`，有 `getStatus/setStatus` | **不存在该接口**。营业状态逻辑直接写在 `controller/admin/ShopController` 里，用 `RedisTemplate`，常量 `public static final String KEY = "SHOP_STATUS"`，值是 `Integer` | `ShopSkill` 必须**注入 `RedisTemplate`** 自行读写 `SHOP_STATUS`，不要去 import 不存在的 Service |
| E2 | Java 是 11/17，langchain4j 选 0.35.0（给 17 用） | 编译级别是 **1.8**。langchain4j 从 **0.36.0 起最低要求 JDK 17** | **`0.35.0` 是可用的最高版本**（已验证其 class 文件 major=52，即 Java 8 字节码）。选 0.36.0+ 必然编译失败 |
| E3 | `TokenStream` 有 `onToolExecuted`，没有才走兜底 | 0.35.0 的 `TokenStream` **只有** `onRetrieved / onNext / onComplete / onError / ignoreErrors / start`，**确定没有 `onToolExecuted`** | `tool_call` / `tool_result` 两类事件**只能由 Skill 方法自己发**，这是主路径不是兜底，直接按 §4.2 设计做 |
| E4 | 工具必须跑在 Tomcat 请求线程 | SSE + `TokenStream` 下工具**必然**在 OkHttp 回调线程执行，这个要求无法满足 | 改为：请求线程捕获 `empId` → 传入 Skill 实例 → 工具方法入口 `BaseContext.setCurrentId(empId)`、`finally` 里 `removeCurrentId()` |
| E5 | `OrderService.confirm` 用确认类 DTO | `confirm(OrdersCancelDTO)`；且 `rejection` / `cancel` 声明 `throws Exception`（**受检**） | Skill 里必须 `catch (Exception e)`，否则编译不过 |
| E6 | 路由图标写 `icon: "inform"`，必要时 `npm run svg` | 侧栏用的是 **iconfont 类名**，必须写 `icon: "icon-inform"`（带 `icon-` 前缀） | **不需要动 `src/icons/` 也不需要跑 `npm run svg`** |

**同时确认为真的两条**：

- ✅ `OrderController.details` 与 `complete` 确实**漏了 `@PathVariable`**（`OrderController.java:59-64`、`108-124`，其中 `delivery` 有而 `details`/`complete` 没有）⇒ **绝不用 RestTemplate 自调 HTTP，一律直调 Service 传 `Long id`**。
- ✅ 补充发现：`OrderServiceImpl` 中 `conditionSearch/statistics/confirm/rejection/cancel/delivery/complete` 这 7 个管理端方法**都没有读 `BaseContext`**（读它的是 `submitOrder/paySuccess/pageQueryForUser/repetition` 等用户端方法）。所以 E4 的线程问题**不会**立刻炸；但 `AutoFillAspect` 对 Dish/Setmeal/Category/Employee 的 Mapper 依赖 `BaseContext`，为了后续扩展和正确性，**仍然必须**按 E4 传播 `empId`。
            ...
            → AgentController                 （只做参数校验 + 建 SseEmitter，零 AI 逻辑）
               → AgentChatService              （LangChain4j AiServices + ChatMemory）
                  → @Tool Skill 实例           （直调下面这层）
                     → OrderService / ReportService / WorkspaceService / RedisTemplate
```
...
这样一次性解决三个问题：① 工具能直接往本次请求的 SSE 里写 `tool_call`/`tool_result`；② `empId` 天然可用，不依赖 ThreadLocal 传播；③ 不同请求互不串扰。

**两条硬约束**（否则工具会静默不注册）：

1. langchain4j 用 `objectWithTools.getClass().getDeclaredMethods()` 扫描工具（已反编译确认）⇒ **`@Tool` 方法必须声明在具体类上，不能靠继承父类拿到**。公共逻辑只能放在被子类调用的 `protected` 辅助方法里。
2. 同理，**Skill 实例不能是 Spring AOP / CGLIB 代理对象**（代理类的 `getDeclaredMethods` 拿不到原方法上的注解）。用 `new` 构造，正好天然规避。
...
public abstract class AbstractSkill {
    ...
    protected String invoke(String toolName, Map<String, Object> args, Callable<String> body) {
        ...
        try {
            result = body.call();
        } catch (BaseException e) {              // 业务异常：把 message 交给模型
            ...
            BaseContext.removeCurrentId();       // Tomcat/OkHttp 线程会复用，必须清理
        }
...
// com.sky.agent.skill.OrderSkill —— @Tool 必须declare在这里
...
> 传递依赖会引入 `dev.ai4j:openai4j`（Retrofit/OkHttp/Jackson）与 `kotlin-stdlib-jdk8:1.9.10`。若与 Spring Boot 2.7 托管的 Jackson 冲突导致 `NoSuchMethodError`，**优先排除 openai4j 里的 jackson 传递依赖**沿用 Boot 托管版本，不要反向升级 Boot。
...
`OpenAiStreamingChatModelBuilder` 在 0.35.0 的可用方法（已反编译确认）：`baseUrl / apiKey / modelName / temperature / topP / maxTokens / timeout(Duration) / logRequests / logResponses / parallelToolCalls / strictTools / customHeaders / listeners`。

### 4.4 记忆

- `ChatMemoryProvider`：key 取 `conversationId`（默认为 `empId` 字符串），value = `MessageWindowChatMemory.withMaxMessages(memoryMaxMessages)`，存在**单例** `ConcurrentHashMap` 里（记忆要跨请求存活，所以这个 Map 必须是单例 Bean，不能每请求新建）。
...
- ❌ 改 `WebMvcConfiguration`（`/admin/**` 已覆盖 `/admin/agent/**`，无需任何改动）
- ❌ 改任何现有 Controller / Service / Mapper / XML 的业务逻辑来「迁就 Agent」（包括**不要**顺手给 `details`/`complete` 补 `@PathVariable`——那是另一个 bug，不在本任务范围）
- ❌ 改 `sky-pojo` 里的实体/DTO/VO
- ❌ 在 `sky-common` / `sky-pojo` 的 pom 加依赖
- ❌ 引入 Redux / Zustand / Pinia / Vuex 新 module；会话状态就放在 `AgentChat.vue` 的组件 `data` 里
- ❌ 引入 Markdown 渲染等重型前端库（构建链太老）；纯文本 + `white-space: pre-wrap` 即可
...
> 每完成一个阶段就 **commit 一次**，commit message 写清阶段号与内容。

### P0 · 预检与基线
- 做完 §1 预检；`git status` 确认工作区干净；先跑一次 `mvn -pl sky-server -am clean package -DskipTests` 记录**改动前**的基线是否可编译。
- **退出标准**：知道基线能否编译（若基线本就编译失败，先记录，不要背锅）。

### P1 · 依赖 + 配置 + Properties
- 改 `pom.xml`、`application.yml`，新建 `AgentProperties`。
- **退出标准**：`mvn -pl sky-server -am clean package -DskipTests` 通过；`mvn dependency:tree` 中出现 langchain4j 0.35.0 且无版本冲突报错。

### P2 · Skill 层（**先不接 LLM**）
- 实现 `AbstractSkill` + 4 个 Skill（§7），`AgentEventSink` 先允许传 `null`（或用一个 no-op 实现）以便单测。
- 写一个 `@SpringBootTest` 或临时 `main`，直接 `new OrderSkill(orderService, noopSink, 1L).searchOrders(...)`，验证能取到**真实数据**、异常被转成中文、列表被截断到 10 条。
- **退出标准**：至少 `searchOrders`、`countOrdersByStatus`、`getTurnover`、`getTodayBusinessData`、`getShopStatus` 五个方法在本地跑出真实结果字符串。**这一步能挡住 90% 的签名/DTO 错误，别跳过。**

### P3 · AiService + 记忆（**先非流式**）
- 先用 `OpenAiChatModel`（非流式）+ `AiServices` 跑通一句「今天营业额怎么样」，确认 DeepSeek 的 function calling 能正确选中工具。
- **退出标准**：控制台能看到工具被调用 + 一段中文回答。
- 若 DeepSeek 的 tool calling 表现不稳，先调系统提示词和 `@Tool` 描述，**不要**改架构。

### P4 · SSE + TokenStream
- `AgentController` + `AgentEventSink`，映射 5 种事件（§8）。`SseEmitter` 超时设 ≥ 120s（建议与 `timeout-seconds` 一致或更长）。
...
### P6 · 自测 + 越界审计 + 文档
- 跑完 §10 测试矩阵。
- `git diff --stat` 逐个文件核对 §5 白名单，**发现越界立即回滚该文件**。
- 写 `docs/` 说明（§11）。
...
- **参数一律用 `String`**（LLM 传参不可靠），Skill 内部再自己 `Long.parseLong` / `LocalDate.parse`，解析失败返回友好中文（如「订单号格式不正确，请提供数字订单 id」），**绝不抛 NPE / NumberFormatException**。
- 日期格式统一 `yyyy-MM-dd`。
- **列表结果最多 10 条**，每条只给：订单号、状态（转成中文）、金额、手机号、下单时间；末尾附「共 N 条，仅显示前 10 条」。
...
| `countOrdersByStatus()` | `statistics()` | 输出待接单/已接单/派送中三个数 |
| `confirmOrder(orderId)` | `confirm(OrdersCancelDTO)` | ⚠ DTO 是 `OrdersCancelDTO`，只塞 `id` |
| `rejectOrder(orderId, reason)` | `rejection(OrdersRejectionDTO)` | `reason` 必填，为空返回「拒单必须说明原因」；⚠ `throws Exception` |
| `cancelOrder(orderId, reason)` | `cancel(OrdersCancelDTO)` | 同上，塞 `cancelReason`；⚠ `throws Exception` |
| `deliverOrder(orderId)` | `delivery(Long)` | — |
| `completeOrder(orderId)` | `complete(Long)` | — |

状态机不合法时（如对已完成订单接单）会抛 `OrderBusinessException`（`BaseException` 子类），由 `AbstractSkill.invoke` 捕获并把 `e.getMessage()` 返回给模型。
...
- VO 里 `dateList`/`turnoverList` 等都是**逗号拼接的字符串**，按下标对齐拼成「2026-08-20 营业额 1234.50 元」这类可读文本。
...
### ShopSkill（注入 `RedisTemplate`，**没有 ShopService**）

- `getShopStatus()`：`(Integer) redisTemplate.opsForValue().get("SHOP_STATUS")`，null 视为未设置；`1`→营业中，`0`→打烊中。
- `setShopStatus(status)`：只接受 `"0"` / `"1"`，其余返回「营业状态只能设置为 0（打烊）或 1（营业）」；写入用 `opsForValue().set("SHOP_STATUS", 整数值)`（**必须是 `Integer`，与现有 Controller 读取时的强转一致**）。
...
- 发送：`emitter.send(SseEmitter.event().data(json))`，`json` 为**单行** JSON 对象（用 fastjson 或 Jackson 序列化；注意 `text` 内容里的换行必须转义，不能真的输出裸换行）
...
- `text` 只发**增量**（对应 `TokenStream.onNext`），不发全量
...
- 关键节点 `log.info`：stream start（含 empId、message 摘要）、tool start / end（含工具名、耗时）、stream end、exception
- 因为并发写风险，`AgentEventSink` 所有发送方法加 `synchronized`
...
### `src/agent/types.ts`
定义事件联合类型。**TS 3.6.2 不支持 `?.` 和 `??`，一个都不许用**（全仓库现在也是 0 处），写显式 `if` 判空。

### `src/agent/sse/agentStream.ts`
- 用**原生 `fetch`**，**不要用 `EventSource`**（无法自定义请求头），**也不要用 `@/utils/request`**（`requestOptimize` 的 md5 去重会取消重复 POST，而且 axios 拿不到流）。
...
- 用 `response.body.getReader()` + `TextDecoder` 增量解码，按 `\n\n` 切分事件块，取出以 `data:` 开头的行，`JSON.parse` 后回调。**注意处理跨 chunk 被切断的半行**（保留 buffer 尾部）。
- 暴露形如 `startAgentStream(message, handlers)` 的函数，`handlers` 含 `onText/onToolCall/onToolResult/onEnd/onError`。

### `src/agent/views/AgentChat.vue`
- `vue-property-decorator` 类组件；**`@Component({ name: 'AgentChat' })` 必须与文件名一致**（`vue/match-component-file-name` 是 error 级）。
- 布局：`el-container`，上方消息列表（用户消息右对齐、助手左对齐），下方 `el-input`（`type="textarea"` 或普通）+ 发送按钮。
- 收到 `text` 就往「当前助手气泡」追加（打字机效果）；正文用 `white-space: pre-wrap`。
- 工具展示：`el-collapse` / `el-card`，显示工具名、参数 JSON（`JSON.stringify(args, null, 2)`）、结果、状态。`tool_call` 到达时插入一张 loading 卡片（`el-icon-loading`），`tool_result` 到达时按 `name` 匹配最近一张未完成的卡片并置为完成。
- 首包未到时显示三个点 CSS 动画。
- 发送后立即清空输入框并 `disabled`，直到 `end` 或 `error` 才恢复。
- `error` 事件用 `this.$message.error(msg)` 提示。
- 样式 `<style lang="scss" scoped>`，参考 `src/views/category/index.vue` 的写法惯例。
...
```ts
{
  ...
  meta: {
    title: "AI 助手",
    icon: "icon-inform"   // ⚠ iconfont 类名，必须带 icon- 前缀；复用现有类，无需新增 svg
  }
}
```
...
| T7 | 营业状态 | 「现在营业吗」→「打烊」 | `getShopStatus` → `setShopStatus("0")`，Redis 中 `SHOP_STATUS` 变为 `0` |
...
> 种子数据 `database/sky.sql:143` 里 admin 的密码是**明文** `123456`，且 `EmployeeServiceImpl.login` 的 MD5 加密行是注释掉的（`EmployeeServiceImpl.java:56-58`），所以直接传明文即可，不要自己先做 MD5。

SSE 观察：`curl -N -X POST http://localhost:8080/admin/agent/stream -H "token: <值>" -H 'Content-Type: application/json' -d '{"message":"今天营业额怎么样"}'`
...
| 找不到 `ShopService` | 该接口不存在 | 直接注入 `RedisTemplate` 操作 `SHOP_STATUS` |
| `rejection`/`cancel` 编译不过 | 它们 `throws Exception` | `catch (Exception e)` |
| 侧栏菜单没图标 | `meta.icon` 写成了 svg 名 | 改成 `icon-inform` 这类 iconfont 类名 |
...
| SSE 在浏览器里不流式、要等全部结束 | 代理或响应被缓冲 | 确认 `produces=text/event-stream`；必要时给 devServer proxy 关掉压缩 |
| 启动就报 Bean 创建失败 | key 为空时强行构造模型 | 改条件装配 / 懒加载（D2 硬要求） |

---

## 13. 执行纪律

1. **先读代码再改代码**。本文件给的签名和行号已核实，但动手前仍应打开对应文件确认上下文。
2. **每阶段 commit 一次**，不要攒一个巨型提交。
3. 遇到与本文件描述不符的情况，**以实际代码为准**，并在最终报告里明确指出「文档第 X 条与实际不符：……」。
4. 不确定的技术选型**不要自由发挥**——本文件已锁定版本和架构，按 §12 的决策表处理，不要为绕开问题而升级框架。
